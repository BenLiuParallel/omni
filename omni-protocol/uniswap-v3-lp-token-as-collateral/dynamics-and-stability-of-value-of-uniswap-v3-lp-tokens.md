...
It is clear from the plot above, that LP token is a safer and less risky asset than its underlying risky asset, as the value of the LP token drops slower than its underlying risky asset when its price drops. Therefore, the LP token collateral has better stability in its value than its underlying risky asset (i.e. blue and orange line are above the grey line when ETH price drops).
